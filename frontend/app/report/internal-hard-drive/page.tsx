...
function extractBrand(name: string): string {
  ...
  if (name.toLowerCase().includes("seagate")) return "Seagate";
    ...
    return (
      <div className="bg-slate-800 border border-slate-600 rounded px-4 py-3 text-xs text-white shadow max-w-xs">
        ...
        <ul className="space-y-0.5">
          ...
          <li><b>RPM:</b> {data.rpm ?? "-"}</li>
          <li><b>Cache:</b> {data.cache ?? "-"} MB</li>
...
export default function InternalHardDriveTierGraphPage() {
  const [hdds, setHdds] = useState([]);
  const [viewMode, setViewMode] = useState("performance");

  useEffect(() => {
    const fetchData = async () => {
      const ref = collection(db, "parts/internal-hard-drive/items");
  ...
  const data = useMemo(() => {
    return hdds
      .filter(hdd => {
        if (viewMode === "performance") return hdd.performance_score;
        return hdd.value_score && hdd.price && hdd.price > 0;
      })
      .map(hdd => {
        const name = safeString(hdd.name);
        const score = viewMode === "performance" ? hdd.performance_score : hdd.value_score;
        const brand = hdd.brand || extractBrand(name);
        const tier = viewMode === "performance" ? hdd.performance_tier : hdd.value_tier;
        let model = name;
        let brandLabel = "";
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white px-4 py-6">
      <h1 className="text-3xl font-extrabold text-center mb-1">Internal Hard Drive Tier Report</h1>
      <p className="text-xs text-center mt-2 text-slate-500 italic">
        {viewMode === "performance" ? (
          <>
            <span className="text-sky-400">Performance = RPM × cache</span><br />
            RPM and Cache size determine the HDD's performance.
          </>
        ) : (
          <>
            <span className="text-purple-400">Value = performance score ÷ price</span><br />
            Value Score shows performance per dollar. HDDs with missing price are excluded.
          </>
        )}
      </p>
